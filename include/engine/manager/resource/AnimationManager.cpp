...
bool AnimationManager::loadAnimation(const std::filesystem::path& jsonPath) {
    ...
    try {
        ...
        std::cerr << "Error parsing animation JSON file " << jsonPath << ": " << e.what() << std::endl;
        return false;
    }
    ...
    for (auto& [animName, animData] : json["animations"].items()) {
        ...
        if (animData.contains("texturePath") && animData["texturePath"].is_string()) {
            texturePath = animData["texturePath"].get<std::string>();
        } else {
            std::cerr << "Error: Animation '" << animName << "' in " << jsonPath << " missing 'texturePath'." << std::endl;
            continue;
        }
        ...
        auto animation = std::make_shared<Animation>(texturePath, loop);
